...
namespace hz {
	...
	struct trap_verifier {
		[[noreturn]] static void ensure_fail(
	...
	template<typename T>
	concept verifier = requires(T verifier) {
		T::ensure_fail("", "", 0, "");
	};

#define hz_assert(expr) ((expr) ? ((void) 0) : Verifier::ensure_fail(#expr, __FILE__, __LINE__, __func__))

	template<typename T, rb_tree_hook (T::*Hook), verifier Verifier = trap_verifier>
	class rb_tree_base {
	public:
	...
	private:
		...
		constexpr void rotate(T* node, bool right) {
			...
			hz_assert(parent && (parent->*Hook).child[!right] == node);
		...
		constexpr void fix_remove(T* node) {
			while (true) {
				...
				hz_assert((parent->*Hook).child[node_is_left]);
				if (auto n = static_cast<T*>((parent->*Hook).child[node_is_left]);
					(n->*Hook).color == 1) {
					rotate(n, !node_is_left);
					hz_assert(node == (parent->*Hook).child[!node_is_left]);
					(parent->*Hook).color = 1;
					(n->*Hook).color = 0;
				}
				...
				hz_assert(is_red(static_cast<T*>((sibling->*Hook).child[node_is_left])));
		...
		constexpr void replace_node(T* node, T* replacement) {
			...
			if (!parent) {
				...
				hz_assert((parent->*Hook).child[1] == node);
		...
		constexpr void remove_half_leaf(T* node, T* child) {
			...
			hz_assert((!(node->*Hook).child[0] && (node->*Hook).child[1] == child)
	...
	template<typename T, rb_tree_hook (T::*Hook), verifier Verifier = trap_verifier> requires requires(T a, T b) {
		a < b;
		a == b;
		a > b;
	}
	class rb_tree : public rb_tree_base<T, Hook, Verifier> {
	public:
		constexpr bool insert(T* node) {
			...
			while (n) {
				prev = n;
				if (*node < *n) {
					n = rb_tree_base<T, Hook>::get_left(n);
				}
				else if (*node > *n) {
					n = rb_tree_base<T, Hook>::get_right(n);
				}
				else {
					return false;
				}
